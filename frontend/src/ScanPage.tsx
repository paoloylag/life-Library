...
function ScanFrame({children}:{children:React.ReactNode}){return <main className="scan-page"><header><button onClick={()=>location.assign('/')} title="Back"><ArrowLeft size={19}/></button><div className="scan-brand"><img src="/lifeos-platform-crest.svg" alt="Life College"/><span><strong>Life College</strong><small>Library Attendance</small></span></div><span className="secure-label"><ShieldCheck size={15}/>Secure</span></header><section className="scan-card">{children}</section><footer>Life College Library · Powered by LifeOS</footer></main>}
