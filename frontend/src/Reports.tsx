...
export default function Reports(){
 ...
 return <div className="reports-page space-y-6">
  ...
  {report&&stats&&<><section className="report-summary-grid">{[
   ...
   <section className="report-panel report-records"><header><h2>Filtered check-ins</h2><p>Showing the first 80 records. Exports include every matching record.</p></header><div className="report-table-wrap"><table><thead><tr><th>Date</th><th>Name</th><th>User type</th><th>Program / Department</th><th>Year / Section</th><th>Check-in time</th><th>Source</th></tr></thead><tbody>{report.records.slice(0,80).map(row=><tr key={row.id}><td>{row.date}</td><td>{row.name}</td><td>{row.category}</td><td>{row.program||row.department||'-'}</td><td>{[row.year_level,row.section].filter(Boolean).join(' / ')||'-'}</td><td>{new Date(row.check_in_time).toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'})}</td><td>{row.source}</td></tr>)}{!report.records.length&&<tr><td colSpan={7} className="report-empty">No records match these filters.</td></tr>}</tbody></table></div></section></>}
 </div>
}
