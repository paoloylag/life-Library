...
export default function Reports(){
 ...
 return <div className="reports-page space-y-6">
  <div className="module-heading"><div><h2>Library Usage</h2><p>Attendance, utilization, and academic-year analysis</p></div><div className="report-export-actions"><button disabled={!report||!!exporting} onClick={()=>download('xlsx')} className="secondary-button"><Download size={16}/>Excel workbook</button><button disabled={!report||!!exporting} onClick={()=>download('pdf')} className="manual-button"><Download size={16}/>PDF report</button></div></div>
  <section className="report-panel report-filters"><div className="report-section-heading"><div><Filter size={18}/>Report filters</div><button onClick={()=>setFilters(initial)} className="report-reset">Reset</button></div><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
  ...
  {error&&<div role="alert" className="module-error">{error}</div>}
  {report&&stats&&<><section className="report-summary-grid">{[
  ...
  ].map(([title,value,detail])=><div key={String(title)} className="report-metric"><p>{title}</p><strong>{value}</strong><small>{detail}</small></div>)}</section>
   <section className="report-panel report-executive"><h2>Executive summary</h2><div className="report-executive-grid"><p><strong>Academic year:</strong> {filters.academicYear==='All'?report.academic_years.join(', ')||'All':filters.academicYear}</p><p><strong>Semester:</strong> {filters.semester==='All'?report.semesters.join(', ')||'All':filters.semester}</p><p><strong>Period:</strong> {report.period_start||'-'} to {report.period_end||'-'}</p><p><strong>Trend grouping:</strong> {filters.grouping}</p><p><strong>Generated:</strong> {new Date(report.generated_at).toLocaleString()}</p><p><strong>Average per user:</strong> {stats.average_per_user}</p></div><p className="report-note">{report.calendar_note} {report.profile_note}</p></section>
   ...
   <section className="report-panel report-records"><header><h2>Filtered check-ins</h2><p>Showing the first 80 records. Exports include every matching record.</p></header><div className="table-wrap report-table-wrap"><table><thead><tr><th>Date</th><th>Name</th><th>User type</th><th>Program / Department</th><th>Year / Section</th><th>Check-in time</th><th>Source</th></tr></thead><tbody>{report.records.slice(0,80).map(row=><tr key={row.id}><td>{row.date}</td><td><strong>{row.name}</strong></td><td><span className="badge user-type-badge">{row.category}</span></td><td>{row.program||row.department||'-'}</td><td>{[row.year_level,row.section].filter(Boolean).join(' / ')||'-'}</td><td>{new Date(row.check_in_time).toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'})}</td><td><span className={row.source==='manual'?'badge manual':'badge inside'}>{row.source}</span></td></tr>)}{!report.records.length&&<tr><td colSpan={7} className="report-empty">No records match these filters.</td></tr>}</tbody></table></div></section></>}
 </div>
}
