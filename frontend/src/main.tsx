...
function App(){
 ...
 React.useEffect(()=>{const fn=()=>setPath(location.pathname);addEventListener('popstate',fn);return()=>removeEventListener('popstate',fn)},[])
 React.useEffect(()=>localStorage.setItem('dark-mode',String(dark)),[dark])
 function navigate(next:string){history.pushState({},'',next);setPath(next);setDrawer(false)}
...
function Brand(){return <div className="brand-lockup"><span className="brand-mark"><img src="/lifeos-platform-crest.svg" alt="LifeOS"/></span><span><strong>Life College</strong><small>Library Attendance</small></span></div>}
